...
public class AStarNPIF extends Algorithm {

    public AStarNPIF(Board board) {
        super(board);
    }
    ...
    public Boolean solve(int maxDepth){
        ...
        Vertex root = new Vertex(this.initialBoard, 0, pastBoards, pastMoves, 4);
        ...
        for(int i=0; i<allMoves.size();i++){
            for(int j=0; j<allMoves.get(i).size();j++){
                ...
                newPastMoves.add(allMoves.get(i).get(j));
                 ...
                 //Create new Vertex
                Vertex newChild = new Vertex(allMoves.get(i).get(j).getNewBoard(), vertex.getDepth()+1, newPastBoards, newPastMoves, 4);
